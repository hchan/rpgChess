function Sacrifice() {
	...
	this.updatePossibleTiles = function() {
		...
		addToPossibleTiles(SELECTEDTILE.getDisplayX(1) + SELECTEDTILE.displayY,
				possibleTiles);
		addToPossibleTiles(
				SELECTEDTILE.getDisplayX(-1) + SELECTEDTILE.displayY,
				possibleTiles);
	...
	this.updateWeightedTurn = function(weightedTurn, tile) {
		...
		if (tile.rank != "king") {
			if (SELECTEDTILE_POSSIBLE_TILES.length > 0) {
				weightedTurn.selectedTile = tile;
				weightedTurn.weight = tile.boardPiece.damage
						* SELECTEDTILE_POSSIBLE_TILES.length;
			}
		}
	}
	...
	this.perform = function() {
		if (SELECTEDTILE_POSSIBLE_TILES.length == 0) {
			$
					.alert("",
							"There are no enemies nearby.  You cannot sacrifice yourself in vain.");
			return;
		}
		if (!getCurrentPlayer().hasEnoughResourceToPerformSkill(this.skillName)) {
			$.alert("",
					"You do not have enough resources to perform this skill");
			return;
		}
		...
		for ( var i = 0; i < SELECTEDTILE_POSSIBLE_TILES.length; i++) {
			...
			// deduct hitpoints
			var dmgRand = SELECTEDTILE.boardPiece.damage
					* (Math.floor((Math.random() * DMG_RAND_PERCENT) + 1))
					/ 100;
			if (Math.random() > 0.5) {
				dmgRand *= -1;
			}
			DMG_AMOUNT = (SELECTEDTILE.boardPiece.damage + dmgRand)
					* this.multiplier;
		...
		SELECTEDTILE.fadeOut();
		var controlPanel = new ControlPanel();
		controlPanel.clearSelectedUnit();
		controlPanel.logSacrifice(SELECTEDTILE);
		SELECTEDTILE.boardPiece = null;
